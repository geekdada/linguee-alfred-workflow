...
func main() {
	base := "https://www.linguee.com/%s/search?qe=%s&source=auto"
	...
	if match == false {
		...
		response.AddItem(&alfred.AlfredResponseItem{
			...
			Uid:   "origin",
			Title: origin,
			Arg:   fmt.Sprintf("https://www.linguee.com/%s/search?source=auto&query=%s", args.Lang, url.QueryEscape(args.Query)),
		})
	}
...
func result(req Request, translation Translation) string {
	...
	default:
		return fmt.Sprintf("https://www.linguee.com%s", translation.Href)
	}
}
...
func filterDocument(doc *goquery.Document) *goquery.Selection {
	doc.Find(".sep, .grammar_info").ReplaceWithHtml("")
...
func parsePhrase(selection *goquery.Selection) (result []string) {
	selection.Find(".translation_item").Each(func(index int, meaning *goquery.Selection) {
		wordType := parseWordType(meaning)
		meaning.Find(".wordtype").ReplaceWithHtml("")

		result = append(result, fmt.Sprintf("%s(%s)", strings.TrimSpace(meaning.Text()), wordType))
	})

	return
}

func parseWordType(selection *goquery.Selection) string {
	return strings.TrimSpace(selection.Find(".wordtype").Text())
}
